...
def draw_bboxes(image, bboxes):
    ...
    for bbox in bboxes:
        ...
        cv2.putText(image, cell_type_label, (int(x1 + 5), int(y1 + 15)),
...
excel_file = f"ground_truth_Blood_Cancer_test.xlsx"
...
output_folder = f"1000_HCM_Microscope_data/v8_bb_2/test_bb_size_GT_Pred/"
input_folder = f"1000_HCM_Microscope_data/v8_bb_2/test_bb_size_pred/"
...
for index, row in df.iterrows():
        ...
        image_filename = f"{input_folder}/{file_name}.png"
        
        #bbox = [row['X1_cord'], row['Y1_cord'], row['X2_cord'], row['Y2_cord']]
        bbox = [row['X1_cord'], row['Y1_cord'], row['X2_cord'], row['Y2_cord'],row['T_Label']]
