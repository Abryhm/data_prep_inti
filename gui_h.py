...
class DoctorApp(QMainWindow):
    ...
    def create_excel_workbook(self):
        ...
        self.excel_path = os.path.join(slide_name,'data.xlsx')
    ...
    def save_data_excl(self):
        ...
        if ret:
            ...
            image_name= f"{slide_name}_{patch_number}_{selected_image_size[0]}_{leukemia_type[0]}"
    ...
    def init_ui(self):
        ...
        lt5_checkbox = QRadioButton("APML")
        ...
        self.capture_image_button = QPushButton("Microscope Capture", self)
        ...
        self.live_feed_label3 = QLabel(self)
        camera_layout.addWidget(self.live_feed_label3)
        ...
        self.live_feed_label2 = QLabel(self)  # Add this label
        camera_layout3.addWidget(self.live_feed_label2)
    ...
    def init_cameras(self):
        ...
        self.camera3 = cv2.VideoCapture(0)
    ...
    def update_live_feed1(self):
        ret, frame = self.camera.read()
        if ret:
            self.update_live_feed_1(frame, self.live_feed_label)

    def update_live_feed2(self):
        ret, frame = self.camera2.read()
        if ret:
             self.update_live_feed(frame, self.live_feed_label2)

    def update_live_feed3(self):
        ret, frame = self.camera3.read()
        if ret:
            self.update_live_feed_without_resize(frame, self.live_feed_label3)

    def update_live_feed4(self):
        ret, frame = self.camera4.read()
        if ret:
            self.update_live_feed(frame, self.live_feed_label4)
    def update_live_feed_1(self, frame, live_feed_label):
