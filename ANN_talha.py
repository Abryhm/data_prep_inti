...
def save_data_to_excel(x1,y1,x2,y2,slidename,patch_number,image):   
        ...
        sheet.cell(row=next_row, column=2).value = patch_number.get()
...
patch_number = tk.Entry(Cell_types_frame)
patch_number.pack(side="left", padx=2, pady=2)
...
Eosinophill_checkbox = tk.Radiobutton(Cell_types_frame, text="E",variable=Cell_types_var, value="Eosinophill")
...
l_checkbox = tk.Radiobutton(Leukemia_Nucleous_frame, text="Prominent",variable=attr_Nucleous_var, value=2)
...
u_checkbox =tk.Radiobutton(Leukemia_Cytoplasmic_vacuoles_frame, text="Prominent",variable=attr_Cytoplasmic_vacuoles_var, value =2)
...
def reset_frame_border_color():
 ...
 #>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
 #>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
image_name_label = tk.Label(sub_frame2, text="", font=('Times New Roman', 35, 'bold'))
...
def end_drawing(event):
    ...
    if current_drawing_box:
        ...
        #idx += 1
        label_x = (bbox_start_x + x) / 2
        label_y = (bbox_start_y + y) / 2
